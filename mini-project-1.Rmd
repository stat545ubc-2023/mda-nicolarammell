...
What are the most common and rare tree species of Vancouver? Do tree communities differ by neighborhood?
...
trees <- datateachr::vancouver_trees             # create object for our tibble
dplyr::glimpse(trees)                            # explore tibble characteristics
summary(trees)                                   # see variable characteristics/NAs
...
Explanation: I will begin with Exercise 3 because the first way I wish to explore the dataset is to investigate the number of NA values for each of the columns. With 146,611 rows of data, scrolling through the observations and noting NA frequency is not feasible. Therefore, it makes sense to calculate the number of NAs using code and plot the proportion of NA data in order to visualize how much of the data is missing.
...
```{r}
### Next, we will plot the proportion of missing data for each variable ###
...
ggplot(trees, aes(year_planted)) +
   geom_histogram(aes(y = after_stat(count)), fill = "tan") +
   theme_classic() +
   xlab("Year") + ylab("Number of trees planted") # note warning: "removed 76548 rows" = NAs
```

Summary: The above plot shows the distribution of the variable `year planted`. We can see that from the beginning of the dataset in 1989, the number of trees that were planted in a given year increased over time. The number of trees planted seems to peak around 1995, after which the number of trees planted plateaus until the early 2010s, after which point the number of trees planted in a given year declines.
...
```{r}
### Next, let's check how many observations per genus there are ###
trees %>%
...
```{r}
### Finally, let's make the density plot for the trunk diameter of Betula species ###
trees %>% 
   filter(genus_name == "BETULA") %>% 
...
2. Do tree community composition differ by neighborhood?  
